...
async function getSnowCredentialsForUser(userId: string) {
  const client = await pool.connect();
  try {
    ...
    if (result.rows.length === 0) {
      throw new Error("ServiceNow credentials are not configured for this user.");
    }
...
async function logTicketAction(
  userId: string,
  action: "create" | "update" | "resolve" | "comment",
  ...
  try {
    ...
    try {
      ...
      const logMessage = `[${action.toUpperCase()}] ticket=${ticketNumber || "N/A"}; ${details}`;
      await client.query(
        'INSERT INTO "faq_agent_logging" ("operation_details", "create_timestamp", "user_name") VALUES ($1, $2, $3)',
        [logMessage, new Date().toISOString(), userName],
      );
    } finally {
      client.release();
    }
  } catch (logErr) {
    console.warn("Could not log ticket action:", logErr);
  }
}

function normalizeSnowBaseUrl(instance: string): string {
  const value = (instance || "dev.service-now.com").trim();
  if (!value) return "https://dev.service-now.com";
...
async function callServiceNowApi(
  ...
  const snowUser = await getSnowCredentialsForUser(userId);
  const baseUrl = normalizeSnowBaseUrl(snowUser.snow_instance);
  const endpoint = query
    ? `${baseUrl}/api/now/table/${tableName}?sysparm_query=${encodeURIComponent(query)}`
  ...
  const response = await fetch(endpoint, {
    ...
    headers: {
      ...
      Authorization:
        "Basic " +
        Buffer.from(
          `${snowUser.snow_username}:${snowUser.snow_password || ""}`,
        ).toString("base64"),
    },
...
app.post("/api/servicenow/create-incident", async (req, res) => {
  ...
  if (!userId || !short_description) {
    return res.status(400).json({ error: "User and short description are required." });
  }
  ...
  try {
    ...
    if (assignment_group) payload.assignment_group = assignment_group;
    if (caller_id) payload.caller_id = caller_id;

    const data = await callServiceNowApi(userId, "POST", "incident", payload);
    const incident = data.result || data;
    const ticketNumber = incident.number || null;
    await logTicketAction(
      userId,
      "create",
      ticketNumber,
      `short_description=${short_description}; category=${payload.category}; priority=${payload.priority}; impact=${payload.impact}; urgency=${payload.urgency}`,
    );
    ...
    console.error("Create ServiceNow incident failed:", err);
    await logTicketAction(userId, "create", null, `ERROR: ${err.message || "Unable to create the incident."}`);
    return res.status(500).json({ error: err.message || "Unable to create the incident." });
  }
});

app.post("/api/servicenow/update-incident", async (req, res) => {
  const { userId, ticketNumber, sysId, updates } = req.body || {};

  if (!userId || (!ticketNumber && !sysId)) {
    return res.status(400).json({ error: "User and ticket identifier are required." });
  }

  try {
    let incidentRecord: any = null;
    if (sysId) {
      incidentRecord = { sys_id: sysId };
    } else {
      const searchResult = await callServiceNowApi(
        userId,
        "GET",
        "incident",
        undefined,
        `number=${encodeURIComponent(ticketNumber)}`,
      );
      const results = Array.isArray(searchResult.result) ? searchResult.result : [];
      if (!results.length) {
        return res.status(404).json({ error: `Ticket ${ticketNumber} was not found.` });
      }
      incidentRecord = results[0];
    }

    const data = await callServiceNowApi(
      userId,
      "PATCH",
      `incident/${incidentRecord.sys_id}`,
      updates || {},
    );
    const ticketRef = incidentRecord.number || ticketNumber || null;
    await logTicketAction(
      userId,
      "update",
      ticketRef,
    ...
    console.error("Update ServiceNow incident failed:", err);
    await logTicketAction(userId, "update", ticketNumber || null, `ERROR: ${err.message || "Unable to update the ticket."}`);
    return res.status(500).json({ error: err.message || "Unable to update the ticket." });
  }
});

app.post("/api/servicenow/resolve-incident", async (req, res) => {
  const { userId, ticketNumber, sysId, resolution_note, close_code } = req.body || {};
  ...
  try {
    ...
    const data = await callServiceNowApi(
      ...
      {
        state: "6",
        close_code: close_code || "Solved Remotely",
        resolution_code: close_code || "Solved Remotely",
        resolution_notes: resolution_note || "Resolved via NextGen AI workflow.",
      },
    );
    const ticketRef = incidentRecord.number || ticketNumber || null;
    await logTicketAction(
      ...
      "resolve",
      ticketRef,
      `close_code=${close_code || "Solved Remotely"}; resolution_note=${(resolution_note || "Resolved via NextGen AI workflow.").slice(0, 250)}`,
    );
    ...
    console.error("Resolve ServiceNow incident failed:", err);
    await logTicketAction(userId, "resolve", ticketNumber || null, `ERROR: ${err.message || "Unable to resolve the ticket."}`);
    return res.status(500).json({ error: err.message || "Unable to resolve the ticket." });
  }
});

app.post("/api/servicenow/comment-incident", async (req, res) => {
  const { userId, ticketNumber, sysId, comment } = req.body || {};

  if (!userId || (!ticketNumber && !sysId) || !comment || !comment.trim()) {
    return res.status(400).json({ error: "User, ticket identifier, and comment text are required." });
  }
  ...
  try {
    ...
    await logTicketAction(
      ...
      "comment",
      ticketRef,
      `work_notes=${comment.trim().slice(0, 250)}`,
    );
    ...
    console.error("Comment ServiceNow incident failed:", err);
    await logTicketAction(userId, "comment", ticketNumber || null, `ERROR: ${err.message || "Unable to add the comment."}`);
    return res.status(500).json({ error: err.message || "Unable to add the comment." });
  }
});
