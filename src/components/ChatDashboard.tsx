...
export function ChatDashboard({ user, onLogout }: ChatDashboardProps) {
  ...
  const tierProgressTimersRef = useRef<number[]>([]);
  const [showTicketAction, setShowTicketAction] = useState<"create" | "update" | "resolve" | "comment" | null>(null);
  ...
  const [updateIncidentForm, setUpdateIncidentForm] = useState({
    ticketNumber: "",
    state: "2",
    priority: "3",
    assigned_to: "",
  ...
  const [resolveIncidentForm, setResolveIncidentForm] = useState({
    ticketNumber: "",
    close_code: "Solved Remotely",
    resolution_note: "",
  });
  const [commentIncidentForm, setCommentIncidentForm] = useState({
    ticketNumber: "",
    comment: "",
  });
  ...
  const clearTierProgressTimers = () => {
    tierProgressTimersRef.current.forEach((timerId) => window.clearTimeout(timerId));
    tierProgressTimersRef.current = [];
  };

  const startTierProgress = () => {
    clearTierProgressTimers();
    const tierSequence = ["Tier 1", "Tier 2", "Tier 3"];

    tierSequence.forEach((tier, index) => {
      const timerId = window.setTimeout(() => {
        setProcessingTier((currentTier) =>
          currentTier === null || currentTier === tier || index === 0
            ? tier
            : currentTier,
        );
      }, index * 1400);

      tierProgressTimersRef.current.push(timerId);
    });
  };
  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      clearTierProgressTimers();
      setProcessingTier("Tier 3");
      setLoadingMessages(false);
    }
  };

  const handleCreateIncident = async () => {
    if (!createIncidentForm.short_description.trim()) {
      alert("Please enter a short description for the incident.");
      return;
    }

    setTicketActionLoading(true);
    try {
      const res = await fetch("/api/servicenow/create-incident", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: userState.user_id,
          short_description: createIncidentForm.short_description,
          description: createIncidentForm.description || createIncidentForm.short_description,
          category: createIncidentForm.category,
          priority: createIncidentForm.priority,
          impact: createIncidentForm.impact,
          urgency: createIncidentForm.urgency,
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Incident creation failed.");

      setExportNotice(`Incident ${data.number || "created"} was created successfully.`);
      setShowTicketAction(null);
      ...
      alert(err.message || "Incident creation failed.");
    } finally {
  ...
  const handleUpdateIncident = async () => {
    if (!updateIncidentForm.ticketNumber.trim()) {
      alert("Please enter a ServiceNow incident number to update.");
      return;
    }
    ...
    try {
      ...
      if (updateIncidentForm.state) payload.state = updateIncidentForm.state;
      if (updateIncidentForm.priority) payload.priority = updateIncidentForm.priority;
      if (updateIncidentForm.assigned_to) payload.assigned_to = updateIncidentForm.assigned_to;
      if (updateIncidentForm.short_description) payload.short_description = updateIncidentForm.short_description;

      const res = await fetch("/api/servicenow/update-incident", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: userState.user_id,
          ticketNumber: updateIncidentForm.ticketNumber,
          updates: payload,
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Ticket update failed.");

      setExportNotice(`Ticket ${updateIncidentForm.ticketNumber} was updated successfully.`);
      setShowTicketAction(null);
      setUpdateIncidentForm({
        ticketNumber: "",
        state: "2",
        priority: "3",
        assigned_to: "",
  ...
  const handleResolveIncident = async () => {
    if (!resolveIncidentForm.ticketNumber.trim()) {
      alert("Please enter a ServiceNow incident number to resolve.");
      return;
    }

    setTicketActionLoading(true);
    try {
      const res = await fetch("/api/servicenow/resolve-incident", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: userState.user_id,
          ticketNumber: resolveIncidentForm.ticketNumber,
          close_code: resolveIncidentForm.close_code,
          resolution_note: resolveIncidentForm.resolution_note || "Resolved by NextGen AI workflow.",
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Ticket resolution failed.");

      setExportNotice(`Ticket ${resolveIncidentForm.ticketNumber} was resolved successfully.`);
      setShowTicketAction(null);
      setResolveIncidentForm({
        ticketNumber: "",
        close_code: "Solved Remotely",
        resolution_note: "",
      });
      ...
      alert(err.message || "Ticket resolution failed.");
    } finally {
      setTicketActionLoading(false);
    }
  };
  ...
  const handleAddComment = async () => {
    if (!commentIncidentForm.ticketNumber.trim()) {
      alert("Please enter a ServiceNow incident number to comment on.");
      return;
    }
    if (!commentIncidentForm.comment.trim()) {
    ...
    setTicketActionLoading(true);
    try {
      const res = await fetch("/api/servicenow/comment-incident", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: userState.user_id,
          ticketNumber: commentIncidentForm.ticketNumber,
          comment: commentIncidentForm.comment,
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Comment update failed.");

      setExportNotice(`Comment added to ticket ${commentIncidentForm.ticketNumber}.`);
      setShowTicketAction(null);
      setCommentIncidentForm({
        ticketNumber: "",
        comment: "",
      });
      setTimeout(() => setExportNotice(null), 4000);
    } catch (err: any) {
      alert(err.message || "Comment update failed.");
    } finally {
      setTicketActionLoading(false);
    }
  };
      ...
      <main className="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        <header className="h-16 px-6 border-b border-[#1a436b]/60 flex items-center justify-between shrink-0 z-10 bg-[#061c31]/80 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setShowTicketAction("create")}
            ...
            <button
              onClick={() => setShowTicketAction("update")}
              className="px-3 py-1.5 text-xs font-medium flex items-center gap-1.5 rounded-lg border border-[#1a436b] bg-[#09223a] hover:bg-[#0e3156] text-slate-200 transition-all cursor-pointer"
            >
            ...
            <button
              onClick={() => setShowTicketAction("resolve")}
              className="px-3 py-1.5 text-xs font-medium flex items-center gap-1.5 rounded-lg border border-[#1a436b] bg-[#09223a] hover:bg-[#0e3156] text-slate-200 transition-all cursor-pointer"
            >
            ...
            <button
              onClick={() => setShowTicketAction("comment")}
              className="px-3 py-1.5 text-xs font-medium flex items-center gap-1.5 rounded-lg border border-[#1a436b] bg-[#09223a] hover:bg-[#0e3156] text-slate-200 transition-all cursor-pointer"
            >
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-xl">
            <div className="bg-[#08223b] p-6 rounded-2xl border border-sky-500/20 shadow-2xl">
              <div className="flex items-center justify-between pb-4 border-b border-[#184063] mb-4">
                <h3 className="font-semibold text-base text-white flex items-center gap-2">
                  {showTicketAction === "create" && <Plus className="w-4 h-4 text-emerald-400" />}
                  {showTicketAction === "update" && <RefreshCw className="w-4 h-4 text-amber-400" />}
                  {showTicketAction === "resolve" && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
                  {showTicketAction === "comment" && <MessageSquare className="w-4 h-4 text-sky-400" />}
                  <span>
                    {showTicketAction === "create" && "Create Incident"}
                    {showTicketAction === "update" && "Update Incident"}
                    {showTicketAction === "resolve" && "Resolve Ticket"}
                    {showTicketAction === "comment" && "Comment on Incident"}
                  </span>
                </h3>
                <button
                  onClick={() => setShowTicketAction(null)}
                  className="p-1 rounded hover:bg-[#0c2b47] text-slate-400 hover:text-white cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              ...
              {showTicketAction === "create" ? (
                <div className="space-y-3 text-xs">
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Short Description</label>
                    <input
                      type="text"
                      value={createIncidentForm.short_description}
                      onChange={(e) => setCreateIncidentForm((prev) => ({ ...prev, short_description: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Description</label>
                    <textarea
                      rows={3}
                      value={createIncidentForm.description}
                      onChange={(e) => setCreateIncidentForm((prev) => ({ ...prev, description: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Category</label>
                      <select
                        value={createIncidentForm.category}
                        onChange={(e) => setCreateIncidentForm((prev) => ({ ...prev, category: e.target.value }))}
                        className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      >
                    ...
                    <div>
                      <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Priority</label>
                      <select
                        value={createIncidentForm.priority}
                        onChange={(e) => setCreateIncidentForm((prev) => ({ ...prev, priority: e.target.value }))}
                        className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Impact</label>
                      <select
                        value={createIncidentForm.impact}
                        onChange={(e) => setCreateIncidentForm((prev) => ({ ...prev, impact: e.target.value }))}
                        className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      >
                    ...
                    <div>
                      <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Urgency</label>
                      <select
                        value={createIncidentForm.urgency}
                        onChange={(e) => setCreateIncidentForm((prev) => ({ ...prev, urgency: e.target.value }))}
                        className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      >
                        <option value="1">1 - High</option>
                        <option value="2">2 - Medium</option>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-white font-semibold text-xs cursor-pointer"
                  >
                    {ticketActionLoading ? "Creating Incident..." : "Create Incident"}
                  </button>
                </div>
              ) : showTicketAction === "update" ? (
                <div className="space-y-3 text-xs">
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Incident Number</label>
                    <input
                      type="text"
                      value={updateIncidentForm.ticketNumber}
                      onChange={(e) => setUpdateIncidentForm((prev) => ({ ...prev, ticketNumber: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      placeholder="INC0007001"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">State</label>
                      <select
                        value={updateIncidentForm.state}
                        onChange={(e) => setUpdateIncidentForm((prev) => ({ ...prev, state: e.target.value }))}
                        className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      >
                    ...
                    <div>
                      <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Priority</label>
                      <select
                        value={updateIncidentForm.priority}
                        onChange={(e) => setUpdateIncidentForm((prev) => ({ ...prev, priority: e.target.value }))}
                        className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      >
                        <option value="1">P1 Critical</option>
                        <option value="2">P2 High</option>
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Assigned To (user name or sys_id)</label>
                    <input
                      type="text"
                      value={updateIncidentForm.assigned_to}
                      onChange={(e) => setUpdateIncidentForm((prev) => ({ ...prev, assigned_to: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      placeholder="user.name or sys_id"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Short Description</label>
                    <input
                      type="text"
                      value={updateIncidentForm.short_description}
                      onChange={(e) => setUpdateIncidentForm((prev) => ({ ...prev, short_description: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                    />
                  </div>
                  <button
                    ...
                    disabled={ticketActionLoading}
                    className="w-full py-2.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-white font-semibold text-xs cursor-pointer"
                  >
                    {ticketActionLoading ? "Updating Ticket..." : "Update Incident"}
                  </button>
                </div>
              ) : showTicketAction === "resolve" ? (
                <div className="space-y-3 text-xs">
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Incident Number</label>
                    <input
                      type="text"
                      value={resolveIncidentForm.ticketNumber}
                      onChange={(e) => setResolveIncidentForm((prev) => ({ ...prev, ticketNumber: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      placeholder="INC0007001"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Resolution Code</label>
                    <select
                      value={resolveIncidentForm.close_code}
                      onChange={(e) => setResolveIncidentForm((prev) => ({ ...prev, close_code: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                    >
                      <option value="Solved Remotely">Solved Remotely</option>
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Resolution Note</label>
                    <textarea
                      rows={3}
                      value={resolveIncidentForm.resolution_note}
                      onChange={(e) => setResolveIncidentForm((prev) => ({ ...prev, resolution_note: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                  ...
                  <button
                    ...
                    className="w-full py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-white font-semibold text-xs cursor-pointer"
                  >
                    {ticketActionLoading ? "Resolving Ticket..." : "Resolve Ticket"}
                  </button>
                </div>
              ) : (
                <div className="space-y-3 text-xs">
                  <div>
                    ...
                    <input
                      type="text"
                      value={commentIncidentForm.ticketNumber}
                      onChange={(e) => setCommentIncidentForm((prev) => ({ ...prev, ticketNumber: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      placeholder="INC0007001"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1 font-mono uppercase">Comment / Work Note</label>
                    <textarea
                      rows={4}
                      value={commentIncidentForm.comment}
                      onChange={(e) => setCommentIncidentForm((prev) => ({ ...prev, comment: e.target.value }))}
                      className="w-full px-3 py-2 rounded-lg bg-[#051C2C] border border-[#184063] text-white focus:outline-none focus:border-[#C5A059]"
                      placeholder="Add the latest update or investigation note"
                    />
                  </div>
                  <button
                    onClick={handleAddComment}
                    disabled={ticketActionLoading}
                    className="w-full py-2.5 rounded-lg bg-sky-500 hover:bg-sky-400 text-white font-semibold text-xs cursor-pointer"
                  >
                    {ticketActionLoading ? "Adding Comment..." : "Add Comment"}
                  </button>
                </div>
              )}
            </div>
